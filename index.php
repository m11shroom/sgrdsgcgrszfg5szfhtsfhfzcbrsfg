...
if (!empty($_SESSION['user_id'])) {
    header('Location: ' . ($_SESSION['is_admin'] ? 'admin.php' : 'dashboard.php')); exit;
}
...
<script type="text/javascript">
    ...
    })(window, document,'script','https://mc.yandex.ru/metrika/tag.js?id=112046530', 'ym');

    ym(112046530, 'init', {ssr:true, webvisor:true, clickmap:true, ecommerce:"dataLayer", referrer: document.referrer, url: location.href, accurateTrackBounce:true, trackLinks:true});
</script>
<noscript><div><img src="https://mc.yandex.ru/watch/112046530" style="position:absolute; left:-9999px;" alt="" /></div></noscript>
<!-- /Yandex.Metrika counter -->
...
.right::before{
  ...
  width:500px;
  height:500px;
  background:radial-gradient(circle, rgba(200,255,0,.03) 0%, transparent 70%);
  pointer-events:none;
}
...
@media(max-width:768px){
  ...
  .left{padding:28px 24px 32px;border-right:none;border-bottom:1px solid var(--border);}
...
<div class="left">
  ...
  <div class="left-content">
    <div class="logo-area">
      <div class="logo-mark">
        <svg width="16" height="16" viewBox="0 0 16 16"><path d="M8 1C4.13 1 1 4.13 1 8s3.13 7 7 7 7-3.13 7-7-3.13-7-7-7zm0 2.5a1.5 1.5 0 110 3 1.5 1.5 0 010-3zm0 8.75c-1.75 0-3.3-.9-4.2-2.26.02-1.39 2.8-2.15 4.2-2.15 1.4 0 4.18.76 4.2 2.15-.9 1.36-2.45 2.26-4.2 2.26z"/></svg>
      </div>
      <div class="logo-name">M1plus wallet</div>
    </div>
    ...
    <div class="hero-text">
      ...
      <p class="hero-sub" data-i18n="hero_sub">wallet.m1plus.ru — платформа для тех,<br>кто ценит контроль над каждой транзакцией.</p>
    </div>

    <div class="stats-row">
      <div class="stat">
        <div class="stat-val">99<span>.9%</span></div>
        <div class="stat-label" data-i18n="stat_uptime_label">Uptime</div>
      </div>
      <div class="stat">
        <div class="stat-val"><span>&lt;</span>50ms</div>
        <div class="stat-label" data-i18n="stat_latency_label">Latency</div>
      </div>
      <div class="stat">
        <div class="stat-val">256<span>bit</span></div>
        <div class="stat-label" data-i18n="stat_encryption_label">Encryption</div>
      </div>
    </div>
  </div>
</div>
...
<div class="right">
  <div class="form-card">
      ...
      <div class="field">
        ...
        <div class="field-wrap">
          <div class="field-icon">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="M22 7L13 13a2 2 0 01-2 0L2 7"/></svg>
          </div>
          <input type="email" name="email" id="emailInput" placeholder="user@m1plus.ru" required autocomplete="off" data-i18n-placeholder="email_placeholder">
        </div>
      </div>
    ...
    <div class="reg-note" data-i18n="reg_note">
      No account yet? Register at <a href="https://m1plus.ru" target="_blank" data-i18n-link="reg_link">m1plus.ru</a>
    </div>
  </div>
</div>
...
const translations = {
  ru: {
    ...
    hero_sub: "wallet.m1plus.ru — платформа для тех,<br>кто ценит контроль над каждой транзакцией.",
    ...
    email_placeholder: "user@m1plus.ru",
    password_label: "Пароль",
    password_placeholder: "••••••••",
    signin_button: "Войти →",
    divider_text: "или продолжить с помощью",
    passkey_button: "Аутентификация по ключу доступа",
    reg_note: "Нет аккаунта? Зарегистрируйтесь на <a href=\"https://m1plus.ru\" target=\"_blank\">m1plus.ru</a>",
  },
  en: {
    hero_eyebrow: "Secure · Private · Fast",
    hero_title: "Your money.<br><span>Fully</span><br>under control.",
    hero_sub: "wallet.m1plus.ru — a platform for those<br>who value control over every transaction.",
    ...
    reg_note: "No account yet? Register at <a href=\"https://m1plus.ru\" target=\"_blank\">m1plus.ru</a>",
  }
};

let currentLang = 'ru'; // start with Russian (since initial text is Russian)
...
function setLanguage(lang) {
  ...
  // translate elements with data-i18n attribute (text content / innerHTML)
  ...
  // special case for passkey button inner HTML (contains icon + text)
  const pkBtn = document.getElementById('pkBtn');
  if (pkBtn && dict.passkey_button) {
    // preserve SVG and replace text node
...
// initial translation to Russian (because default visible texts are English but we need Russian as primary)
setLanguage('ru');
document.getElementById('langBtn').textContent = 'EN';

// attach event listener to lang button
...
(function(){
  ...
  function build(){
    pts=[];
    var cols=Math.ceil(W/60)+1, rows=Math.ceil(H/60)+1;
    grid={cols,rows,w:W/(cols-1),h:H/(rows-1)};
    for(var r=0;r<rows;r++){
      for(var c=0;c<cols;c++){
        pts.push({
          bx:c*grid.w, by:r*grid.h,
          ox:(Math.random()-.5)*22, oy:(Math.random()-.5)*22,
          ox2:0, oy2:0,
          vx:0, vy:0,
  ...
  function draw(){
    ...
    for(var i=0;i<pts.length;i++){
      ...
      p.ox2+=(Math.sin(t*.7+p.phase)*18-p.ox2)*.03;
      p.oy2+=(Math.cos(t*.5+p.phase)*18-p.oy2)*.03;

      var dx=mouse.x-(p.bx+p.ox);
      var dy=mouse.y-(p.by+p.oy);
      var dist=Math.sqrt(dx*dx+dy*dy);
      var rep=Math.max(0,1-dist/130)*14;
      p.ox2-=dx/dist*rep*.3;
      p.oy2-=dy/dist*rep*.3;
    }
    ...
    for(var r=0;r<grid.rows;r++){
      for(var c=0;c<grid.cols;c++){
        var i=r*grid.cols+c;
        var p=pts[i];
        var px=p.bx+p.ox+p.ox2, py=p.by+p.oy+p.oy2;

        if(c<grid.cols-1){
          var p2=pts[i+1];
          var px2=p2.bx+p2.ox+p2.ox2, py2=p2.by+p2.oy+p2.oy2;
          var g=ctx.createLinearGradient(px,py,px2,py2);
          g.addColorStop(0,'rgba(200,255,0,.07)');
          g.addColorStop(1,'rgba(200,255,0,.04)');
          ctx.strokeStyle=g;
          ctx.beginPath();ctx.moveTo(px,py);ctx.lineTo(px2,py2);ctx.stroke();
        }
        if(r<grid.rows-1){
          var p3=pts[i+grid.cols];
          var px3=p3.bx+p3.ox+p3.ox2, py3=p3.by+p3.oy+p3.oy2;
          var g2=ctx.createLinearGradient(px,py,px3,py3);
          g2.addColorStop(0,'rgba(200,255,0,.07)');
          g2.addColorStop(1,'rgba(200,255,0,.02)');
          ctx.strokeStyle=g2;
          ctx.beginPath();ctx.moveTo(px,py);ctx.lineTo(px3,py3);ctx.stroke();
        }

        // dots at intersections
        var a=.08+Math.abs(Math.sin(t+p.phase))*.15;
        ctx.fillStyle=`rgba(200,255,0,${a.toFixed(2)})`;
